	...
	class Glf_Mor_Widget extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			if ( ! isset( $instance[ "has_ordering" ] ) || $instance[ "has_ordering" ] ) {
				$text .= do_shortcode( apply_filters( 'widget_text', Glf_Utils::glf_mor_get_shortcode( isset( $instance[ "ruid" ] ) ? $instance[ "ruid" ] : "", 'ordering', isset( $instance[ 'use_custom_css' ] ) ? $instance[ 'use_custom_css' ] : 0, isset( $instance[ 'class' ] ) ? $instance[ 'class' ] : '' ), $instance, $this ) );
			}

			if ( ! isset( $instance[ "has_reservations" ] ) || $instance[ "has_reservations" ] ) {
				$text .= do_shortcode( apply_filters( 'widget_text', Glf_Utils::glf_mor_get_shortcode( isset( $instance[ "ruid" ] ) ? $instance[ "ruid" ] : "", 'reservations', isset( $instance[ 'use_custom_css' ] ) ? $instance[ 'use_custom_css' ] : 0, isset( $instance[ 'class' ] ) ? $instance[ 'class' ] : '' ), $instance, $this ) );
			}
		...
		public function form( $instance ) {
			$restaurants = Glf_Utils::glf_mor_get_restaurants();
            ...
            </label><select name="<?php echo esc_attr( $this->get_field_name( 'ruid' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'ruid' ) ); ?>">
